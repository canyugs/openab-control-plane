...
| `OABCP_PLANE_STATUS_NOTICE` | _(off)_ | `1` enables the ADR 025 requester notice: when a review session dies verdict-less (timeout with a dead chair), the plane itself posts a canned "review could not complete" PR comment via the App. The plane's only direct GitHub write; default-off |
