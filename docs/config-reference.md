...
| `OABCP_SESSION_TIMEOUT_SECS` | `600` | Liveness watchdog deadline. A session with no new message for this many seconds is force-closed with a `TIMEOUT` verdict and a north `timeout` event so a silent/dead council can't hang forever. Anchored on the latest message (or session creation before the first message). Keep an attached OpenAB bot's `pool.prompt_hard_timeout_secs`, its liveness-check precision, and delivery acknowledgement ceiling strictly below this value; the shipped OCP configs use 480s + 30s + 5s = 515s |
