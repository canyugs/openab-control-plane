...
| **Post-review actions** — chair posts action menu, compact summary (🔴×1 🟡×10 🟢×5) | **✅ v1** | Final PR comment ends with a summary + action footer: counts, `/ask` follow-up, push-to-re-run. Chair-side prompt change (tmpl + steering + skill) |
| **Commit status target_url** — Checks tab "Details" links to the review comment | **✅ v1** | Chair sets `openab/council` commit status on the head SHA (`success`/`failure`, description = counts, `target_url` = review comment). Needs `Commit statuses: Read and write` (install docs updated) |
