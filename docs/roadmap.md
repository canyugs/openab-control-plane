...
| **Control plugins + OAB Father** — package use cases as installable control plugins and manage them through an operator surface | **✅ extraction (Stage 3)** / OAB Father TODO | [ADR 007](adr/007-control-plugins-and-oab-father.md) executed as the Stage 3 extraction ([ADR 018](adr/018-stage3-extraction.md), S1–S17): PR review + triage live in `src/plugins/` behind the four `Coordinator` hooks + `controller::execute`; kernel purity is CI-gated (grep gate + mode-enumeration test); the S16 second-consumer test proves the seam on trait defaults with zero `plugins::` imports; B1/B3 closed, B2 frozen + demotion runbook (ADR 010), D1 done (ADR 016 blockers 1–3, S14/S15). Verdict-column **storage** moves at M4 on the ADR 018 seam. [ADR 008](adr/008-external-controller-protocol.md) transport stays dormant (reserved verbs + serialized shapes pinned in ADR 018); OAB Father is not started |
...
| **Audit log / message observability** — first-party, queryable investigation record across the provider controller and kernel | proposed | [ADR 017](adr/017-message-observability-audit-layer.md) established durable message/runtime audit; [ADR 036](adr/036-first-party-investigation-journal.md) refines it after the ADR 031 extraction. Messages and session logs are durable. External-controller runtime events have a durable queue and delivery state for configured controller sessions, but do not yet form a complete retained investigation history. The remaining gap is a shared correlation contract plus append-only local journals for all material lifecycle, retry, and provider-side-effect outcomes. Physical stores stay separate and are composed at read time; no third-party telemetry dependency or provider vocabulary in the kernel. |
