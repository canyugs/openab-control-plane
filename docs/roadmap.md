...
| **Shared steering via `pre_seed`** — review output format + rules delivered to bots as an object-store layer (`shared/default.tar.gz`), not stuffed in every trigger | **TODO (deferred)** | OAB's job, not the plane's (see [design scope](design.md)). Mechanism: OAB `[hooks.pre_seed]` layer concept — base shared layer + per-agent override. Use **Cloudflare R2 / any S3-compatible store via `endpoint_url`** (our users are on Zeabur/Cloudflare, not AWS). Ref: OAB `docs/hooks.md`. Package `docs/steering/pr-review.md` into the shared archive. **Investigated 2026-06-27:** `pre_seed` confirmed built-in (default-on), R2-compatible via `endpoint_url`, layered + SHA-256-checksummed — it landed in OAB **0.9.0-beta.3** (the latest release; `git diff beta.2..beta.3` touched no gateway-wire / bot-config files, so the OCP plane's wire contract is unaffected). The standing council's bots were bumped **beta.2 → beta.3-claude**, so the capability is now live and validated. **Deferred** because trigger-based steering is already proven through dogfood webhook reviews and the copied Action path, and the review format is already consistent *without* a pre_seed layer. Do it once another trigger builder would otherwise duplicate the steering text, or to iterate the house style centrally. **Storage backend (where the archive lives / who serves it) is an open decision — see [ADR 003](adr/003-steering-delivery.md): external R2 vs an OCP-hosted S3 origin (the latter blocked on an OAB `force_path_style` change).** |
...
| Item | Status | Notes |
|------|--------|-------|
| **Bot discovery (membership plane)** — dynamic registration, capability advertisement, health-aware roster; join/leave as first-class events | TODO | Moves membership from boot-time-static `OABCP_BOTS` to a dynamic registry. Runtime replacement now exists (`POST /v1/sessions/:id/roster/replace`, `POST /v1/council/roster/replace`), but provider/capability/health inventory is still TODO |
...
| **Control plugins + OAB Father** — package use cases as installable control plugins and manage them through an operator surface | proposed | [ADR 007](adr/007-control-plugins-and-oab-father.md): PR review becomes the first plugin/profile on top of the OCP runtime kernel; OAB Father creates/configures/installs plugins but is not the session hot path. [ADR 008](adr/008-external-controller-protocol.md) sketches the external controller event/action protocol |
