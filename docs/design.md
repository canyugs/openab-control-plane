...
| PR-specific logic (gh pr diff, gh pr comment, label) | `github-pr-controller`, a separate crate and process | B1 is closed: the grandfathered in-kernel review product (`/v1/review`, webhook parsing, presets, prompt fabrication) was deleted in v0.1.67 once both lanes showed zero use (ADR 031 invariant #9) |
