...
| `GITHUB_CONTROLLER_BOT_HANDLE` | _(none)_ | Bot handle without `@`. An **alias**: the App's own slug is probed at startup and is what the bot answers to and advertises, so a rename needs no config change. This value keeps working alongside it, and is the sole handle when the probe fails. `/readyz`'s `bot_identity` names the handle in force |
