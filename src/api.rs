...
bot_username = {name}
# OCP interprets delivered content as session control. Send only the
# ACP-terminal content so session close freezes the canonical transcript.
streaming = false
...
max_sessions = 4
session_ttl_hours = 2
# Keep the OAB turn ceiling below OCP's default 600s silence watchdog.
# 480s + the 30s check interval + the 5s gateway ACK cap leaves 85s margin.
