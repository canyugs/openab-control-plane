...
| **Clean template** — TEMPLATE.md + one-click deploy, no manual bot registration | ✅ | `seed_bot` on boot + `npx zeabur template deploy` (3 vars, no manual bot registration) + [TEMPLATE.md](TEMPLATE.md) marketplace page with two install tracks (PAT + copied Action / GitHub App webhook) |
...
| **Auto-trigger** — webhook shim: PR opened / `/review` comment → open session | ✅ | The GitHub App webhook (`POST /api/v1/github_webhooks`) convenes a real council on PR `opened`/`reopened`/`ready_for_review` or a `/review` comment (v0.1.6, #14). The dogfood repo no longer carries a repo-local `council-review.yml`; the copied Action remains only as `examples/pr-review.yml` for external/PAT installs, and manual reruns use `scripts/open-council.sh`. Pre-prod gaps: no per-repo allowlist / `/review` perm gate |
