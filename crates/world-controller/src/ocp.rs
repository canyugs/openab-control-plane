//! Scoped OCP action client, `github-pr-controller/src/ocp.rs` generalized to
//! one `execute` over any `ControllerAction` — this controller needs
//! `open_session` and `post_message`.
...
pub trait OcpActionClient: Send + Sync {
    fn execute(&self, action_id: String, action: ControllerAction) -> ActionFuture;
}
...
impl std::fmt::Display for ActionFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Unavailable => write!(f, "ocp_action_unavailable"),
            Self::InvalidResponse => write!(f, "ocp_action_invalid_response"),
            Self::Protocol { status, code, .. } => {
                write!(f, "ocp_action_rejected (http {status}, {code:?})")
            }
        }
    }
}
...
mod tests {
    ...
    fn action_client_requires_a_safe_complete_origin() {
        let complete = |url: &str| OcpActionConfig {
            ...
            scope: Some("tenant:dev/resource:world".into()),
            controller_id: Some("world-dev".into()),
        };
