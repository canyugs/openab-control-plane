//! Signed runtime-event verification, same contract as
//! `github-pr-controller/src/runtime_events.rs` with `session.intent`
//! (ADR 039) added to the supported set.
...
mod tests {
    ...
    #[test]
    fn session_intent_events_verify_end_to_end() {
        let secret = vec![7; 32];
        let verifier =
            RuntimeEventVerifier::new("world-dev", &URL_SAFE_NO_PAD.encode(&secret)).unwrap();
        let target = "/api/v1/openab/events?version=1";
        let body = br#"{"version":"1","event_id":"cev_1","controller_id":"world-dev","event_type":"session.intent","session_id":"ses_1","occurred_at":1000000,"payload":{"message_id":"msg_1","bot_id":"b0","verb":"delegate","args":"to=b1 task=\"x\""}}"#;
        ...
        let event = verifier
            ...
            .unwrap();
        assert_eq!(event.event_type, "session.intent");
